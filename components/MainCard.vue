  ...
  <div class="rounded-lg bg-white grid grid-cols-5 max-h-[336px] overflow-hidden hover:shadow-md">
    <div class="col-span-2 flex flex-col h-[336px] gap-1">
      <div v-for="(image, index) in item.images" :key="index" :class="imageClass()">
        <NuxtImg :src="`/img/${image}`" :class="['w-full h-full object-cover image-overflow', nuxtImgClass(index)]"
          :alt="item.title" quality="100" />
      </div>
    </div>
    <div class="col-span-3 flex flex-col p-[40px_30px_34px_34px] space-y-3">
      ...
      <ul v-show="item.list?.length" class="list-disc list-inside marker:text-gold">
        <li v-for="listItem in item.list" :key="listItem" class="font-nunitoSans font-bold text-sm text-slateGray">{{
          listItem }}</li>
      </ul>
      <p v-for="desc in item.description" :key="desc" class="font-nunitoSans font-semibold">
        {{ desc }}
      </p>
    </div>
  </div>
